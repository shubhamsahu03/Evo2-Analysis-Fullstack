...
evo2_image = (
    ...
    # STEP 1: Install PyTorch for the correct CUDA version. This is the foundational
    # dependency and must be installed before libraries that depend on it.
    # The 'cu121' wheel from PyTorch is compatible with CUDA 12.x.
    .run_commands(
        "pip3 install torch --index-url https://download.pytorch.org/whl/cu121"
    )

    # STEP 2: Install build-time dependencies for flash-attn.
    .pip_install("packaging")

    # STEP 3: Now that torch and packaging are installed, build flash-attn from source.
    # It will find the existing torch installation and build against it.
    .run_commands(
        "pip install flash-attn --no-build-isolation"
    )

    # STEP 4: Finally, install the main application and its other dependencies.
    # Pip will see that torch and flash-attn are already installed and use them.
